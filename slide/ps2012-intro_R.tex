...
t = 0.1574, df = 80.38, p-value = 0.8753
...
\begin{itemize}
\item R functions are stored in packages.
...
\item However, before you can load a package, you have to install (download)
      it. This can be done via \texttt{install.packages("meta")}.
\item Whenever a new R session is started, the packages have to be loaded via \texttt{library(...)}.
\end{itemize}
...
\end{frame}
\section{Installation, help and maintenance}
\label{sec-2}
\subsection{}
\begin{frame}
...
\begin{enumerate}
\item Choose a CRAN mirror (e.g., University of Tennessee, Knoxville, TN: \href{http://mira.sunsite.utk.edu/CRAN/}{http://mira.sunsite.utk.edu/CRAN/}).
\item Choose a precompiled binary distribution (e.g., Windows).
\item Choose binaries for base distribution and then \enquote{Download R 2.12.0 for Windows}.
\end{enumerate}
\item After downloading R-2.12.0-win.exe, execute the file.
\end{itemize}
\end{frame}
\begin{frame}
...
\end{frame}
\begin{frame}
\frametitle{Text editors (or: How to interact with R)}
\label{sec-2-1-4}

    Use a text editor which (at least) offers syntax highlighting. 
\begin{itemize}
\item A recommended solution is RStudio (see next slide; can be downloaded from XXX)
...
\begin{verbatim}
                    x         y sex
     1  0.3030082  3.711373   f
     2  0.1472815  9.260739   m
     3 -1.4600131 13.344104   f
     4  0.8182952  6.155728   m
\end{verbatim}
\end{frame}
...
\begin{verbatim}
                x                 y           sex  
      Min.   :-1.4600   Min.   : 0.3861   f:5  
      1st Qu.:-1.0771   1st Qu.: 6.2413   m:5  
      Median :-0.1659   Median : 8.8817        
      Mean   :-0.2580   Mean   : 8.5273        
      3rd Qu.: 0.3590   3rd Qu.:12.3132        
      Max.   : 1.1605   Max.   :13.3441
\end{verbatim}
...
\begin{verbatim}
     ...
     -1.4600 -1.0770 -0.1659 -0.2580  0.3590  1.1610
\end{verbatim}
\end{frame}
\begin{frame}[fragile]

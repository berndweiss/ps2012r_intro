...
\begin{itemize}
\item To sort a vector, use the function \texttt{sort()}, e.g.
\end{itemize}
...
\begin{itemize}
\item However, when it comes to sorting a data frame, \texttt{sort()} does not
      work. Instead, use the function \texttt{order()}, which returns an index, which
...
\begin{verbatim}
                    x         y sex
     1  0.4917055  9.333354   f
     2  1.0703966  6.761825   m
     3  0.1769224  9.630359   f
     4 -0.8990933 16.440533   m
\end{verbatim}
\end{frame}
...
\begin{verbatim}
      x                  y         
Min.   :-2.63846   Min.   : 5.418  
1st Qu.:-1.09749   1st Qu.: 8.702  
Median :-0.09656   Median : 9.545  
Mean   :-0.37925   Mean   :10.909  
3rd Qu.: 0.46036   3rd Qu.:13.993  
Max.   : 1.07040   Max.   :16.441  
sex  
f:5  
m:5
\end{verbatim}
...
\begin{verbatim}
     ...
     -2.63800 -1.09700 -0.09656 -0.37920 
      3rd Qu.     Max. 
      0.46040  1.07000
\end{verbatim}
\end{frame}
...
\end{frame}
\begin{frame}[fragile,shrink = 5]
\frametitle{Identifying duplicates I}
\label{sec-7-1-2}


    The function \texttt{duplicated()} is handy to identify duplicate elements of a
...
\end{frame}
\begin{frame}[fragile]
\frametitle{Identifying duplicates II and \texttt{unique()}}
\label{sec-7-1-3}


\begin{itemize}
\item The result of a \texttt{duplicated()} can be used to exclude double entries.
\end{itemize}
...
\begin{itemize}
\item However, to get a list of unique entries just use \texttt{unique()}.
\end{itemize}
...
\label{sec-9-2}
\begin{frame}
\frametitle{\ldots{} to be continued \ldots{}}
\label{sec-9-2-1}

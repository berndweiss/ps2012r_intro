...
*** Packages
    ...
    - However, before you can load a package, you have to install (download)
      it. This can be done via =install.packages("meta")=.
...
*** Download and installation
    - R can be downloaded from the Comprehensive R Archive Network (CRAN). The process is as follows:
      1. Choose a CRAN mirror (e.g., University of Tennessee, Knoxville, TN: http://mira.sunsite.utk.edu/CRAN/).
      2. Choose a precompiled binary distribution (e.g., Windows).
      3. Choose binaries for base distribution and then "Download R 2.12.0 for Windows".
    - After downloading R-2.12.0-win.exe, execute the file. 
